...
class TestVkinder:
    ...
    # Тест проверки поиска фотографий
    @pytest.mark.parametrize('user_id, result', [('336261035', True)])
    def test_get_photo(self, user_id, result):
        assert get_photo(user_id) == result

    # Тест проверки сортировки по лайкам
    ...
    # Тест проверки первичной регистрации пользователя
    @pytest.mark.parametrize('vk_id, result', [('1', False), ('1', False), ('336261035', False)])
    def test_register_user(self, vk_id, result):
        assert register_user(vk_id) == result

    # Тест проверки добавление пользователя
    @pytest.mark.parametrize('event_id, vk_id, first_name, second_name, city, link, id_user, result',
                             [('7717001', '2', 'George', 'Boris', 'Australia', 'www.vkman.ru', '1', False)])
    ...
    # Тест проверки добавление фотографии анкеты в базу данных
    ...
    # Тест проверки добавления в черный список
    @pytest.mark.parametrize(
        'event_id, vk_id, first_name, second_name, city, link, link_photo, count_likes, id_user, result',
        [('123', '12', '12434', '1251231', 'kpsdfsdfsrt', 'kpsfsdfsdfdsrt', 'kpfsdfsdfsrt', '212', '5123', False)])
