...
def menu_bot(id_num):
    write_msg(id_num,
              f"Чат бот - VKinder\n"
              f"\nПройдите регистрацию в случае использования впервые.\n"
              f"Для регистрации введите - Да.\n"
              f"Если вы уже зарегистрированы - начинайте поиск.\n"
              f"\nДля поиска - девушка 18-35 Екатеринбург\n"
    ...
    while True:
        ...
        if msg_text == "vkinder":
            ...
            # Поиск партнера
                ...
                sex = 0
                if msg_text[0:7].lower() == 'девушка/женщина':
                    sex = 1
                ...
                if int(age_to) >= 100:
                    write_msg(user_id, 'максимальное значение возвраста установлено 99 лет.')
                    age_to = 99
                ...
                for i in range(len(result)):
                    ...
                    # Получение фото, сортировка по лайкам
                    user_photo = get_photo(result[i][3])
                    if user_photo == 'отсутствует доступ к фотографиям' or dating_user is not None or blocked_user is not None:
                        continue
                    sorted_user_photo = sort_likes(user_photo)
                    # Вывод отсортированных данных по анкетам
